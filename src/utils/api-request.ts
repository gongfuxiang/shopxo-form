...
const service = axios.create({
    baseURL: import.meta.env.VITE_APP_BASE_API_PHP == '/dev-php' ? import.meta.env.VITE_APP_BASE_API_PHP : pro_url + '/api.php?s=',
    timeout: 60000,
...
service.interceptors.response.use(
    (response: AxiosResponse) => {
        ...
        if (response.config.url?.includes('?s=user/userverifyentry')) {
